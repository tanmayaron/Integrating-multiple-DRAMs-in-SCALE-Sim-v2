...
class read_buffer:
    def __init__(self):
        ...
        self.backing_buffer1 = read_port()
        self.backing_buffer2 = read_port()
        ...
        self.trace_matrix1 = np.ones((1, 1))
        self.trace_matrix2 = np.ones((1, 1))
    ...
    def reset(self): # TODO: check if all resets are working propoerly
        ...
        # Variables to enable prefetching
        self.fetch_matrix = np.ones((1, 1))
        self.fetch_matrix1 = np.ones((1, 1))
        self.fetch_matrix2 = np.ones((1, 1))
        self.last_prefect_cycle = -1
        self.next_line_prefetch_idx = 0
        self.next_col_prefetch_idx = 0
    ...
    def set_fetch_matrix(self, fetch_matrix_np):
        ...
        self.fetch_matrix1 = np.ones((num_lines, self.req_gen_bandwidth)) * -1
        self.fetch_matrix2 = np.ones((num_lines, self.req_gen_bandwidth)) * -1
        ...
        for i in range(num_elems):
            ...
            self.fetch_matrix1[dest_row][dest_col] = fetch_matrix_np[src_row][src_col]
    ...
    def prepare_hashed_buffer(self):
        ...
        prefetch_rows1 = self.fetch_matrix1.shape[0]
        prefetch_cols1 = self.fetch_matrix1.shape[1]
        ...
        for r in range(prefetch_rows1):
            for c in range(prefetch_cols1):
                elem = self.fetch_matrix1[r][c]
    ...
    # Depending on the size of the active buffer, calculate the number of lines from op mat to fetch
    ...
    # 1. Preparing the requests:
        ...
        if not num_lines < self.fetch_matrix1.shape[0]:
            num_lines = self.fetch_matrix1.shape[0]
        ...
        for i, row in enumerate(self.fetch_matrix1[start_idx:end_idx, :]):
            if i % 2 == 0:
    ...
    # 1.1 See if extra requests are made, if so nullify them
        ...
        #        prefetch_requests[row][col] = -1
                if row<=num1:
                    prefetch_requests1[row][col] = -1
                if row<=num2:
                    prefetch_requests2[row][col] = -1
    ...
    #    The start_cycle variable ensures that all the requests have been made before any incoming reads came
        cycles_arr1 = np.zeros((num_lines, 1))
        for i in range(cycles_arr1.shape[0]):
            cycles_arr1[i][0] = -1 * (num_lines - start_cycle - (i - self.backing_buffer1.get_latency()))

        cycles_arr2 = np.zeros((num_lines, 1))
        for i in range(cycles_arr2.shape[0]):
            cycles_arr2[i][0] = -1 * (num_lines - start_cycle - (i - self.backing_buffer2.get_latency()))

    # 3. Send the request and get the response cycles count for each fetch matrix
    ...
    # 4. Update the variables for fetch matrix 1
        # Fill prefetch_requests1 with rows of -1s if it has fewer rows than response_cycles_arr1
        if len(response_cycles_arr1) > len(prefetch_requests1):
            num_rows_to_fill = len(response_cycles_arr1) - len(prefetch_requests1)
            rows_of_minus_ones = np.full((1, prefetch_requests1.shape[1]), -1)
            for _ in range(num_rows_to_fill):
                prefetch_requests1 = np.vstack((prefetch_requests1, rows_of_minus_ones))

        if len(response_cycles_arr2) > len(prefetch_requests2):
            num_rows_to_fill = len(response_cycles_arr2) - len(prefetch_requests2)
            rows_of_minus_ones = np.full((1, prefetch_requests2.shape[1]), -1)
            for _ in range(num_rows_to_fill):
                prefetch_requests2 = np.vstack((prefetch_requests2, rows_of_minus_ones))

        # Concatenate response_cycles_arr1 and prefetch_requests1
        ...
        # 5. Update the variables for fetch matrix 2
    ...
    # The module operator is to ensure that the indices wrap around
        if requested_data_size > self.active_buf_size:  # Some elements in the current idx are left out in this case
            self.next_line_prefetch_idx = num_lines % self.fetch_matrix1.shape[0]
        else:
            self.next_line_prefetch_idx = (num_lines + 1) % self.fetch_matrix1.shape[0]


    def new_prefetch(self):
        # In a new prefetch, some portion of the original data needs to be deleted to accommodate the prefetched data
        ...
        # 3. Divide the prefetch requests into two buffers alternatively
        prefetch_requests1 = []
        prefetch_requests2 = []
        
        print("prefetch is ", prefetch_requests, "\n")
        ...
        # 4. Modify the prefetch request to drop unwanted addresses
        # a. Chomp the elements in the first line included in previous fetches
        for i in range(0, self.next_col_prefetch_idx):
            prefetch_requests1[0][i] = -1
            prefetch_requests2[0][i] = -1

        # b. Chomp the excess elements in the last line
        if requested_data_size > self.active_buf_size:
            valid_cols = int(self.active_buf_size % self.req_gen_bandwidth)
            row = prefetch_requests1.shape[0] - 1
            for col in range(valid_cols, self.req_gen_bandwidth):
                prefetch_requests1[row][col] = -1
                prefetch_requests2[row][col] = -1

        # 5. Create the request cycles
        cycles_arr1 = np.zeros((num_lines, 1))
        for i in range(cycles_arr1.shape[0]):
            # Fixing ISSUE #14
            # cycles_arr[i][0] = self.last_prefect_cycle + i
            cycles_arr1[i][0] = self.last_prefect_cycle1 + i + 1

        cycles_arr2 = np.zeros((num_lines, 1))
        for i in range(cycles_arr2.shape[0]):
            # Fixing ISSUE #14
            # cycles_arr[i][0] = self.last_prefect_cycle + i
            cycles_arr2[i][0] = self.last_prefect_cycle2 + i + 1

        # 6. Send the request
        self.last_prefect_cycle1 = response_cycles_arr1[-1][0]

        assert response_cycles_arr1.shape == cycles_arr1.shape, 'The request and response cycles dims do not match'

        this_prefetch_trace1 = np.concatenate((response_cycles_arr1, prefetch_requests1), axis=1)
        self.trace_matrix1 = np.concatenate((self.trace_matrix1, this_prefetch_trace1), axis=0)


        response_cycles_arr1 = self.backing_buffer.service_reads(incoming_cycles_arr=cycles_arr1,
                                                                incoming_requests_arr_np=prefetch_requests1)
        ...
        response_cycles_arr2 = self.backing_buffer.service_reads(incoming_cycles_arr=cycles_arr2,
                                                                incoming_requests_arr_np=prefetch_requests2)
        
        self.last_prefect_cycle2 = response_cycles_arr2[-1][0]

        assert response_cycles_arr2.shape == cycles_arr2.shape, 'The request and response cycles dims do not match'

        this_prefetch_trace2 = np.concatenate((response_cycles_arr2, prefetch_requests2), axis=1)
        
        self.trace_matrix2 = np.concatenate((self.trace_matrix2, this_prefetch_trace2), axis=0)

        # 7. Update the variables
        self.last_prefect_cycle = max(response_cycles_arr1[-1][0], response_cycles_arr2[-1][0])



    #
    def new_prefetch1(self):
    # In a new prefetch, some portion of the original data needs to be deleted to accommodate the prefetched data
    # In this case we overwrite some data in the active buffer with the prefetched data
    # And then create a new prefetch request
    # Also return when the prefetched data was made available
    ...
    # 2. Create the request for fetch matrix 1
        start_idx = self.next_line_prefetch_idx
        num_lines = math.ceil(self.prefetch_buf_size / self.req_gen_bandwidth)
        end_idx = start_idx + num_lines
        requested_data_size = num_lines * self.req_gen_bandwidth
    ...
    # In case we need to circle back
        if end_idx > self.fetch_matrix1.shape[0]:
            last_idx = self.fetch_matrix1.shape[0]
            prefetch_requests1 = self.fetch_matrix1[start_idx:, :]

            new_end_idx = min(end_idx - last_idx, start_idx)  # In case the entire array is engulfed
            prefetch_requests1 = np.concatenate((prefetch_requests1, self.fetch_matrix1[:new_end_idx, :]))
        else:
            prefetch_requests1 = self.fetch_matrix1[start_idx:end_idx, :]

    # 2. Create the request for fetch matrix 2
        if end_idx > self.fetch_matrix2.shape[0]:
            last_idx = self.fetch_matrix2.shape[0]
            prefetch_requests2 = self.fetch_matrix2[start_idx:, :]

            new_end_idx = min(end_idx - last_idx, start_idx)  # In case the entire array is engulfed
            prefetch_requests2 = np.concatenate((prefetch_requests2, self.fetch_matrix2[:new_end_idx, :]))
        else:
            prefetch_requests2 = self.fetch_matrix2[start_idx:end_idx, :]

    # Modify the prefetch request to drop unwanted addresses for fetch matrix 1
    # a. Chomp the elements in the first line included in previous fetches
        for i in range(0, self.next_col_prefetch_idx):
            prefetch_requests1[0][i] = -1

    # b. Chomp the excess elements in the last line for fetch matrix 1
        if requested_data_size > self.active_buf_size:
            valid_cols = int(self.active_buf_size % self.req_gen_bandwidth)
            row = prefetch_requests1.shape[0] - 1
            for col in range(valid_cols, self.req_gen_bandwidth):
    ...
    # Modify the prefetch request to drop unwanted addresses for fetch matrix 2
    # a. Chomp the elements in the first line included in previous fetches
        for i in range(0, self.next_col_prefetch_idx):
            prefetch_requests2[0][i] = -1

    # b. Chomp the excess elements in the last line for fetch matrix 2
        if requested_data_size > self.active_buf_size:
            valid_cols = int(self.active_buf_size % self.req_gen_bandwidth)
            row = prefetch_requests2.shape[0] - 1
            for col in range(valid_cols, self.req_gen_bandwidth):
                prefetch_requests2[row][col] = -1
    ...
    # 4. Send the request for fetch matrix 1
        response_cycles_arr1 = self.backing_buffer.service_reads(incoming_cycles_arr=cycles_arr,
                                                              incoming_requests_arr_np=prefetch_requests1)

    # 5. Update the variables for fetch matrix 1
        self.last_prefect_cycle1 = response_cycles_arr1[-1][0]

        assert response_cycles_arr1.shape == cycles_arr.shape, 'The request and response cycles dims do not match'

        this_prefetch_trace1 = np.concatenate((response_cycles_arr1, prefetch_requests1), axis=1)
        self.trace_matrix1 = np.concatenate((self.trace_matrix1, this_prefetch_trace1), axis=0)

    # 6. Send the request for fetch matrix 2
        response_cycles_arr2 = self.backing_buffer.service_reads(incoming_cycles_arr=cycles_arr,
                                                              incoming_requests_arr_np=prefetch_requests2)

    # 7. Update the variables for fetch matrix 2
        self.last_prefect_cycle2 = response_cycles_arr2[-1][0]

        assert response_cycles_arr2.shape == cycles_arr.shape, 'The request and response cycles dims do not match'
    ...
    # Set the line to be prefetched next
        if requested_data_size > self.active_buf_size:
            ...
            self.next_line_prefetch_idx = (num_lines + 1) % self.fetch_matrix1.shape[1]
    ...
    def get_external_access_start_stop_cycles(self):
        assert self.trace_valid, 'Traces not ready yet'
        start_cycle = self.trace_matrix1[0][0]
        end_cycle = self.trace_matrix1[-1][0]
    ...
    def print_trace_1(self, filename):
        ...
        num_rows_to_send = len(self.trace_matrix) // 2

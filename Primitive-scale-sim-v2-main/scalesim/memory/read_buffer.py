...
class read_buffer:
    ...
    def service_reads(self, incoming_requests_arr_np,   # 2D array with the requests
        ...
        #print(incoming_requests_arr_np.shape[0])
    ...
    def prefetch_active_buffer(self, start_cycle):
        ...
        for i, row in enumerate(prefetch_requests):
            ...
            prefetch_requests1.append(arr1)
            prefetch_requests2.append(arr2)
            num1+=1
            num2+=1
        ...
        #    valid_cols = floor(valid_cols/2)-1
        #    row = end_idx - 1
        ...
        cycles_arr1 = np.zeros((num1, 1))
        for i in range(cycles_arr1.shape[0]):
            cycles_arr1[i][0] = -1 * (num1 - start_cycle - (i - self.backing_buffer1.get_latency()))

        cycles_arr2 = np.zeros((num2, 1))
        for i in range(cycles_arr2.shape[0]):
            cycles_arr2[i][0] = -1 * (num2 - start_cycle - (i - self.backing_buffer2.get_latency()))
    ...
    def get_num_accesses(self):
        ...
        #Halving the number of accesses as 2 accesses are performed in parallel
        self.num_access=self.num_access//2
